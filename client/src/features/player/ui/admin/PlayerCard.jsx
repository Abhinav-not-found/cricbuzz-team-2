  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden relative">
      ...
      <div ref={ref} className="absolute top-3 right-3">
        ...
        {isOpen && (
          <div className="absolute right-0 top-9 w-32 bg-white border border-gray-200 rounded-xl shadow-md z-10 overflow-hidden">
            <button
              type="button"
              onClick={() => { setOpenId(null); onEdit(player._id) }}
            ...
            <button
              type="button"
              onClick={() => { setOpenId(null); onDelete(player._id) }}
      ...
      <div className="flex flex-col items-center px-4 pt-5 pb-4 border-b border-gray-100">
        ...
        <p className="text-[14px] font-medium text-gray-900 text-center">{player.name}</p>
        <p className="text-[12px] text-gray-400 mt-0.5">{player.country}</p>
        <span className={cn('mt-2 text-[10px] font-medium rounded-md px-2 py-0.5', roleConfig[player.role])}>
          {player.role}
        </span>
      </div>
      ...
      <div className="px-4 py-3 flex flex-col gap-2.5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[12px] text-gray-700">{player.battingStyle || '—'}</span>
        </div>
        <div className="h-px bg-gray-100" />
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-gray-400">Bowling</span>
          <span className="text-[12px] text-gray-700">{player.bowlingStyle || '—'}</span>
        </div>
      </div>
